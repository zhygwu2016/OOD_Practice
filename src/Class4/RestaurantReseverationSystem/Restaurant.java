...
import static Class4.RestaurantReseverationSystem.TableType.*;
    ...
    private class ReservationSplitter {
        ...
        public List<String> splitReservation(@NotNull final Reservation reservation){
            ...
            while(originalSize > 0) {
                ...
                if(originalSize > MEDIUM.getCapacity()) {
                    final List<String> tempList = mRestaurantTableFilter.filteredTables(LARGE,timePeriod);
                    if(!tempList.isEmpty()) {
                        for(String tempId : tempList){
                            if(!set.contains(id)){
                                tableList.add(tempId);
                                originalSize -= LARGE.getCapacity();
                                set.add(tempId);
                                break;
                            }
                        }
                ...
                if(originalSize > SMALL.getCapacity()) {
                    final List<String> tempList = mRestaurantTableFilter.filteredTables(MEDIUM,timePeriod);
                    if(!tempList.isEmpty()) {
                        for(String tempId : tempList){
                            if(!set.contains(id)){
                                tableList.add(tempId);
                                originalSize -= MEDIUM.getCapacity();
                                set.add(tempId);
                                break;
                            }
                        }
                ...
                final List<String> tempList = mRestaurantTableFilter.filteredTables(SMALL,timePeriod);
                if(!tempList.isEmpty()) {
                    for(String tempId : tempList){
                        if(!set.contains(id)){
                            tableList.add(tempId);
