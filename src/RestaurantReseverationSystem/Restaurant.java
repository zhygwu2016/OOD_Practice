        ...
        public List<String> splitReservation(@NotNull final Reservation reservation){
            ...
            while(originalSize > 0) {
                ...
                if(originalSize > MEDIUM.getCapacity()) {
                    final List<String> tempList = mReservationHandler.filteredTables(LARGE,timePeriod);
                    if(!tempList.isEmpty()) {
                ...
                if(originalSize > SMALL.getCapacity()) {
                    final List<String> tempList = mReservationHandler.filteredTables(MEDIUM,timePeriod);
                    if(!tempList.isEmpty()) {
                        for(String tempId : tempList){
                            if(!set.contains(id)){
                                tableList.add(tempId);
                ...
                final List<String> tempList = mReservationHandler.filteredTables(SMALL,timePeriod);
                if(!tempList.isEmpty()) {
                    for(String tempId : tempList){
                        if(!set.contains(id)){
                            tableList.add(tempId);
